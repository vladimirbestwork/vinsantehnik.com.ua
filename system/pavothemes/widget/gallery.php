...
class PtsWidgetGallery extends PtsWidgetPageBuilder {
		...
		public static function getWidgetInfo(){
			return array('label' =>  ('Gallery Image'), 'explain' => 'Create Images Mini Gallery From Folder', 'group' => 'image'  );
		}
		...
		//	d( $this->token );
			$string = ' 
					 ...
					 <script type="text/javascript">
						$(".imageupload").WPO_Gallery({key:"'.$key.'",gallery:true,placehold:"'.$placeholder.'",baseurl:"'.HTTP_CATALOG . 'image/'.'" } );
					</script>
		...
		public function renderContent(  $args, $setting ){
			...
			if( count($size) == 2 ){
			 	...
			 	foreach ($setting['imagefile'] as $image) {
					$setting['thumbnailurl'][] = $this->model_tool_image->resize( $image, (int)$size[0], (int)$size[1],'w');
				}
			}
