...
/* OC-Store: Badges */
				...
				$badges = array();
				foreach ($this->model_ocstore_badges->badges() as $b)
					$badges[$b['id']] = array("image" => $b['image'], "data" => json_decode($b['data'], true));
				$badges_all = $this->model_ocstore_badges->badges_all();
...
/* OC-Store: Badges */
				$this->load->language('module/ocstore_badges');
				$this->load->model('ocstore/badges');
				$data['tab_badges'] = $this->language->get('tab_badges');
				$data['entry_badges'] = $this->language->get('entry_badges');
				$data['badges'] = $this->model_ocstore_badges->badges();
				if (isset($this->request->post['product_layout'])) {
					$data['badges_product'] = $this->request->post['badges'];
				} elseif (isset($this->request->get['product_id'])) {
					$data['badges_product'] = $this->model_ocstore_badges->badges_product($this->request->get['product_id']);
				} else {
					$data['badges_product'] = array();
				}
				/* /OC-Store: Badges */
