<?php
class ControllerShippingautolux extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('shipping/autolux');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/autolux', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('shipping/autolux', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['autolux_total'])) {
			$data['autolux_total'] = $this->request->post['autolux_total'];
		} else {
			$data['autolux_total'] = $this->config->get('autolux_total');
		}
		...
		$this->response->setOutput($this->load->view('shipping/autolux.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'shipping/autolux')) {
