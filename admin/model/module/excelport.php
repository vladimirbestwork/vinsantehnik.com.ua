		...
		switch ($type) {
			case 'Products'	: {
				...
				if ($this->determineProductBulk($objPHPExcel)) {
					...
					$this->load->model('module/excelport_product_bulk');
					$this->model_module_excelport_product_bulk->importXLSProductsBulk($language, $languages, $file, $settings['ImportLimit'], $addAsNew);
				} else {
            ...
            case 'Coupons' : {
                $this->load->model('module/excelport_coupon');
                $this->model_module_excelport_coupon->importXLSCoupons($file, $settings['ImportLimit']);
            } break;
            case 'Vouchers' : {
                $this->load->model('module/excelport_voucher');
                $this->model_module_excelport_voucher->importXLSVouchers($file, $settings['ImportLimit']);
            } break;
		}
	}
		...
		switch ($type) {
			case 'Products'	: {
				...
				if ($exportMode == 0) { // Bulk export
					$this->model_module_excelport_product_bulk->exportXLSProductsBulk($language, $store, $destinationFolder, $settings['ExportLimit'], !empty($filters['Products']) && $filter ? $filters['Products'] : array(), $settings['DescriptionEncoding']);
				} else if ($exportMode == 1) { // Quick export
					$this->model_module_excelport_product->exportXLSProductsLight($language, $store, $destinationFolder, $settings['ExportLimit'], !empty($filters['Products']) && $filter ? $filters['Products'] : array(), $settings['DescriptionEncoding']);
				} else { // Full export
					$this->model_module_excelport_product->exportXLSProductsFull($language, $store, $destinationFolder, $settings['ExportLimit'], !empty($filters['Products']) && $filter ? $filters['Products'] : array(), $settings['DescriptionEncoding']);
				}
			} break;
			case 'Categories' : {
				$this->load->model('module/excelport_category');
            ...
            case 'Coupons' : {
                $this->load->model('module/excelport_coupon');
                $this->model_module_excelport_coupon->exportXLSCoupons($destinationFolder, $settings['ExportLimit'], !empty($filters['Coupons']) && $filter ? $filters['Coupons'] : array());   
            } break;
            case 'Vouchers' : {
                $this->load->model('module/excelport_voucher');
                $this->model_module_excelport_voucher->exportXLSVouchers($destinationFolder, $settings['ExportLimit'], !empty($filters['Vouchers']) && $filter ? $filters['Vouchers'] : array());   
            } break;
		}
	}
